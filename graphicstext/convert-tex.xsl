...
\chapter{List of Sample Programs}
<xsl:apply-templates/>
</xsl:template>
...
<xsl:template match="demoref"><xsl:choose>
   <xsl:when test="text()">\demoref{<xsl:apply-templates/>}</xsl:when>
   <xsl:otherwise><xsl:variable name="a" select="str:new(@href)"/><xsl:variable name="b" select="str:replaceAll($a,'_','\\_')"/>\demoref{<xsl:value-of select="$b"/>}</xsl:otherwise>
</xsl:choose></xsl:template>
...
<xsl:template match="localref">
     <xsl:choose>
         ...
         <xsl:when test="name(id(@href))='appendix'"><xsl:text>Appendix~\ref{</xsl:text><xsl:value-of select="@href"/><xsl:text>}</xsl:text></xsl:when>
