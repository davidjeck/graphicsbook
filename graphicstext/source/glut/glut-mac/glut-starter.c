 ...
 * For example, in MacOS with XCode developer tools:
 *
 *        clang -DGL_SILENCE_DEPRECATION -o executableProg glut-starter.c -framework OpenGL -framework GLUT
 */

#include <OpenGL/gl.h>
#include <GLUT/glut.h>   // Consider using freeglut.h instead, if available.
...
void reshape(int w, int h) {
    ...
    height = h;
    glViewport(0,0,w*PIXEL_SCALE,h*PIXEL_SCALE);  // If you have a reshape function, you MUST call glViewport!
        // PIXEL_SCALE, defined in pixel-scale.h, is for adapting to high-resolution screens on MacOS.
